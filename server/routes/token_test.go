...
func TestTokenHandler_JwtGuard_SetsUserOnSuccessfulWithWebsocket(t *testing.T) {
	t.Parallel()
	// Arrange
	mockJwtService := &MockJwtService{
		validateTokenReturns: &jwt.Token{
	...
	context, _ := gin.CreateTestContext(writer)
	context.Request, _ = http.NewRequest("", "", nil)
	context.Request.Header = http.Header{"Sec-Websocket-Protocol": []string{"Bearer abc"}}
